...
public class ballsballs extends SequentialCommandGroup {
  /** Creates a new ballsballs. */

  private double startTime;

  public ballsballs(String pathName) {
    ...
    addCommands(
      ...
      new RunCommand(() -> SharkExecutor.executeNextAvailableStep(pathName, System.currentTimeMillis()/1000. - startTime), DriveTrain.getInstance()).until(() -> SharkExecutor.isFinished(pathName))
      // new RunCommand(() -> SharkExecutor.executeNextAvailableStep(pathName, System.currentTimeMillis()/1000. - startTime), DriveTrain.getInstance().until(() -> SharkExecutor.isFinished(pathName)));
    );
  }
